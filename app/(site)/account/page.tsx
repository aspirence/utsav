  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
      <div>
        ...
        <p className="text-ink-700 mt-3 max-w-prose leading-relaxed">
          Your number is your account — there is no password to keep. Anything you enquire about
          from now on is tied to it, so you can come back to the replies from any phone or laptop.
        </p>

        <dl className="border-ink-100 mt-8 space-y-4 border-t pt-6">
          <div>
            <dt className="text-ink-500 text-xs font-semibold tracking-[0.14em] uppercase">
              Mobile
            </dt>
            <dd className="text-ink-900 mt-1">{profile?.phone ?? user?.phone ?? '—'}</dd>
          </div>
          <div>
            <dt className="text-ink-500 text-xs font-semibold tracking-[0.14em] uppercase">
              Email
            ...
            <dd className="text-ink-900 mt-1">
              {profile?.email ?? user?.email ?? <span className="text-ink-500">Not added yet</span>}
            </dd>
          </div>
        </dl>
        ...
        <Link
          ...
          className="bg-primary-600 hover:bg-primary-700 mt-8 inline-flex items-center gap-2 rounded-md px-5 py-3 font-medium text-white transition-colors"
        >
      ...
      <aside className="border-ink-100 bg-surface-sunken/50 rounded-2xl border p-6">
        <h3 className="font-display text-ink-900 text-lg">Your planning</h3>
        ...
        <ul className="mt-5 space-y-1">
          {[
            ...
            <li key={row.href}>
              <Link
                href={row.href}
                className="border-ink-100 text-ink-800 hover:text-primary-700 flex items-center justify-between gap-4 border-b py-3 transition-colors"
              >
                <span>{row.label}</span>
                <span className="font-display text-ink-900 text-xl">{row.n}</span>
              </Link>
            </li>
          ))}
        </ul>

        <p className="text-ink-600 mt-5 text-sm leading-relaxed">
          Bookings and reviews arrive with escrow — plan §14. A review needs a completed booking
          behind it, which is why they come together.
        </p>
      </aside>
    </div>
  )
