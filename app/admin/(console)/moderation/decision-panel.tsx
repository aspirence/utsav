      ...
      <div className="space-y-3">
        <div>
          <button
            ...
            className="bg-success-600 hover:bg-success-700 h-10 w-full rounded-lg text-sm font-medium text-white transition-colors disabled:pointer-events-none disabled:opacity-50"
          >
            {job === 'approve' ? 'Approving…' : 'Approve'}
          </button>
          <p className="text-ink-500 mt-1.5 text-xs">{copy.approve}</p>
        </div>
        ...
        <div>
          <button
            ...
            className="border-ink-200 text-ink-800 hover:bg-ink-50 h-10 w-full rounded-lg border bg-white text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50"
          >
            {showReject ? 'Cancel rejection' : 'Reject with reason'}
          </button>

          {showReject ? (
            <div className="border-ink-200 bg-ink-50 mt-2 rounded-lg border p-3">
              <label htmlFor="rejection-reason" className="text-ink-700 text-xs font-medium">
              ...
              <textarea
                ...
                className="border-ink-200 text-ink-900 placeholder:text-ink-400 focus:border-primary-500 mt-1.5 w-full rounded-md border bg-white px-3 py-2 text-sm focus:outline-none"
              />
              <p className="text-ink-500 mt-1 text-xs">
                The vendor reads this verbatim, so write it as if to them. One sentence is enough;
                at least ten characters are required.
              </p>
              <button
                type="button"
                disabled={busy || !reasonReady}
                onClick={() => run('reject', () => rejectItem(itemId, reason))}
                className="bg-danger-500 hover:bg-danger-700 mt-2 h-9 rounded-lg px-4 text-sm font-medium text-white transition-colors disabled:pointer-events-none disabled:opacity-50"
              >
                {job === 'reject' ? 'Rejecting…' : 'Confirm rejection'}
              </button>
            </div>
          ) : (
            <p className="text-ink-500 mt-1.5 text-xs">{copy.reject}</p>
          )}
        </div>
